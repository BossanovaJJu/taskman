...
> 
> 싱글 페이지라고 해도 여러 화면이 필요하기 때문에 각 주소가 필요하다. 결국 다른 주소에 따라 다른 뷰를 보여주는 **라우팅** 기능이 필요하지만 리액트 자체에는 이 기능이 내장되어 있지 않다. 그래서 그 기능을 도와주는 라이브러리 `react-router-dom`이 필요한 것이라고 한다. (브라우저에서만 사용되는 리액트 라우터이다.)
